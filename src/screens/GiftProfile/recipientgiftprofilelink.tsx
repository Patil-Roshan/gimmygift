...
const RecipientGiftProfileLink = () => {
  ...
  const getHighlightedPreferences = async () => {
    try {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <TouchableOpacity
          onPress={() => {
            if (navigation?.getState()?.routes[0]?.name === 'splash') {
              navigation.reset({
                index: 0,
...
export default RecipientGiftProfileLink;
